...
export default async function StatistiquesCafopPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <PageHeader titre="Statistiques CAFOP" description="Effectifs des promotions d'élèves-maîtres." />

      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard libelle="CAFOP" valeur={kpis.cafops} icone={<GraduationCap size={22} />} />
      ...
      <Card>
        <h2 className="mb-4 font-display text-base font-bold text-forest-900">Élèves-maîtres par CAFOP</h2>
        <ChartBarVertical data={graph} nomSerie="Élèves-maîtres" vide="Aucun CAFOP enregistré." />
      </Card>

      <Card>
        <h2 className="mb-3 font-display text-base font-bold text-forest-900">Détail</h2>
        {lignes.length === 0 ? (
          <p className="text-sm text-ink-700/60">Aucun CAFOP enregistré.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-cream-200 text-left text-xs text-ink-700/65">
                  <th className="py-2.5 pr-3 font-semibold">CAFOP</th>
